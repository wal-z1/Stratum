...
import { Download, ExternalLink, FlaskConical, Play } from "lucide-react";
...
export function SampleBrowser({ onAnalyze, isLoading }: SampleBrowserProps) {
	...
	const visibleSamples = showAll ? sampleCaptures : sampleCaptures.slice(0, 6);

	return (
		<section id="samples" className="space-y-4" aria-labelledby="samples-title">
			<div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
				...
				<p className="max-w-xl text-sm leading-6 text-[var(--muted)]">
					Samples are provided by the Wireshark project. Stratum does not host
					or claim ownership of these captures.
				</p>
			</div>
			<div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
				{visibleSamples.map((sample) => (
					<Card
						...
						<CardContent className="flex min-h-40 h-full flex-col gap-2 p-3">
							...
							{sample.fetchMode === "direct" ? (
								<p className="flex items-center gap-1 text-[11px] text-[var(--muted)]">
									<FlaskConical className="h-3 w-3" /> Direct fetch from
									Wireshark
								</p>
							) : null}
						</CardContent>
			...
			{sampleCaptures.length > 6 ? (
				<button
					...
					{showAll
						? "Show fewer samples"
						: `Show all ${sampleCaptures.length} samples`}
				</button>
			) : null}
		</section>
	);
